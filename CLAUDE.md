...
pnpm build        # Production build (TypeScript errors enforced)
...
**Budget Partner** (branding in `lib/site.ts`) is a full-stack financial dashboard. Stack: Next.js 16 App Router + Turbopack, React 19, Supabase (PostgreSQL + Auth + RLS), Tailwind CSS v3, Radix UI/shadcn primitives, React Hook Form + Zod, Recharts, Zustand, GSAP, Stripe, Lucide React.
...
| `app/actions/` | All server actions (`"use server"`). One file per domain: `accounts.ts`, `account-page.ts`, `auth.ts`, `billing.ts`, `credit-cards.ts`, `dashboard.ts`, `invoice.ts`, `merchants.ts`, `notification.ts`, `profile.ts`, `recipient.ts`, `saving-plans.ts`, `transaction.ts`. |
...
| `lib/site.ts` | `SITE_NAME` and `SITE_LOGO_PATH` — use these instead of hardcoding the app name/logo. |
| `lib/safe-redirect.ts` | `safeRedirect(origin, path)` — use for all redirects to prevent open redirects. |
| `supabase/migrations/` | Ordered SQL migrations. Apply via Supabase SQL Editor or `supabase db push`. |

### Route guard

`proxy.ts` (project root) is the Next.js 16 route guard — **not** `middleware.ts`. It protects `/dashboard/*` (redirects unauthenticated users to `/login?next=…`) and redirects authenticated users away from `/login` and `/register`.
...
Supabase Auth. `app/login/` and `app/register/` use server actions (`app/actions/auth.ts`). The dashboard is protected at `app/dashboard/layout.tsx`. Additional pages: `/forgot-password` and `/reset-password`. Google OAuth is optional.
